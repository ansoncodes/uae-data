...
<section id="learn" class="section-y relative bg-surface">
  <div class="container-x">
    ...
    <div data-journey class="relative mt-8 lg:mt-10">
      ...
      <ol class="relative grid gap-9 lg:grid-cols-5 lg:gap-6">
        ${stages
          .map((s, i) => {
          ...
          <span class="journey-node absolute left-0 top-0 z-10 inline-flex size-12 items-center justify-center rounded-full border border-line-strong bg-white font-mono text-[13.5px] font-semibold text-muted lg:relative lg:mx-auto lg:flex">${pad2(i + 1)}</span>
          <div class="lg:mt-8 lg:text-center">
            <h3 class="journey-verb font-display text-[22px] font-bold leading-none tracking-[-0.045em] text-ink">${esc(s.verb)}</h3>
          ...
          <ul class="mt-5 space-y-2.5">
            ${items.map((t) => `<li class="flex gap-2.5 text-[13.5px] leading-relaxed text-muted">${check()}<span>${esc(t)}</span></li>`).join("\n            ")}
          </ul>
        </li>`;
          })
          .join("\n        ")}
    ...
    <div data-reveal class="mt-9 flex flex-col gap-6 rounded-[20px] bg-ink p-6 text-white sm:p-7 lg:mt-14 lg:flex-row lg:items-center lg:justify-between">
      <p class="font-display text-[16px] font-semibold tracking-[-0.02em]">Throughout the programme</p>
      <ul class="flex flex-col gap-3 lg:flex-row lg:gap-8">
        ${learnJourney.alwaysOutcomes
          .map((o) => `<li class="flex gap-2.5 text-[14px] leading-relaxed text-white/75">${check("text-accent")}<span>${esc(course.outcomes[o])}</span></li>`)
          .join("\n        ")}
      </ul>
    </div>
  </div>
...
function unitBody(unit) {
  return `<div class="pb-6">
    ${unit.note ? `<p class="mb-4 inline-flex items-start gap-2 rounded-xl bg-accent-50 px-3.5 py-2.5 text-[13.5px] font-medium text-accent">${icon("rocket", "mt-0.5 size-4 shrink-0")}${esc(unit.note)}</p>` : ""}
    ${
      unit.topics
    ...
    ${
      unit.groups
        ? `<div class="grid gap-3 sm:grid-cols-2">${unit.groups
          ...
          <p class="font-display text-[14px] font-semibold leading-snug text-ink">${esc(g.heading)}</p>
...
function unitAccordion(unit, j, m) {
  ...
  return `<div data-acc-item class="border-t border-line first:border-t-0 ${open ? "is-open" : ""}">
    <button type="button" data-acc-trigger aria-expanded="${open}" aria-controls="${id}" class="flex w-full items-center gap-4 py-5 text-left">
      ...
      <span class="flex-1 font-display text-[15px] font-semibold leading-snug text-ink sm:text-[15.5px]">${esc(unit.title)}</span>
...
function moduleArticle(m, i) {
  ...
  const navBtn = "group inline-flex items-center gap-2 rounded-full px-1 py-2 text-[13.5px] font-semibold text-muted transition-colors hover:text-ink";

  return `<article id="module-${m.id}" data-module="${i}" class="overflow-hidden rounded-[18px] bg-white ring-1 ring-line lg:rounded-[24px] lg:shadow-soft ${active ? "is-active is-open" : ""}">
    <button type="button" data-module-toggle aria-expanded="${active}" aria-controls="module-body-${m.id}" class="module-toggle flex w-full items-center gap-4 p-5 text-left sm:p-5 lg:hidden">
      <span class="inline-flex size-9 shrink-0 items-center justify-center rounded-2xl bg-ink font-mono text-[13px] font-semibold text-white">${pad2(i + 1)}</span>
      <span class="min-w-0 flex-1">
        <span class="block font-display text-[15px] font-semibold leading-snug text-ink">${esc(short(m))}</span>
    ...
    <div id="module-body-${m.id}" data-module-body class="module-body">
      <div class="border-t border-line p-5 sm:p-6 lg:border-t-0 lg:p-8">
        ...
        ${m.kicker ? `<p class="mt-6 text-[13.5px] font-medium text-accent">${esc(m.kicker)}</p>` : ""}
        <h3 class="${m.kicker ? "mt-1.5" : "mt-6"} font-display text-[clamp(1.2rem,1.6vw,1.5rem)] font-bold leading-[1.08] tracking-[-0.035em] text-ink">${esc(m.title)}</h3>
        <p class="mt-3 max-w-2xl text-[14.5px] leading-relaxed text-muted">${esc(m.summary)}</p>
...
<section id="curriculum" class="section-y relative bg-surface">
  <div class="container-x">
    ...
    <div data-curriculum data-reveal class="mt-8 grid gap-8 lg:mt-10 lg:grid-cols-12 lg:gap-8">
      <aside class="hidden lg:col-span-4 lg:block">
        <div class="sticky top-24">
          ...
          <ol class="mt-5 space-y-1">
            ${curriculum
              ...
              <button type="button" data-module-tab="${i}" aria-controls="module-${m.id}" ${i === 0 ? 'aria-current="true"' : ""} class="module-tab group flex w-full items-center gap-4 rounded-2xl px-4 py-3.5 text-left text-muted hover:text-ink ${i === 0 ? "is-active" : ""}">
                ...
                <span class="min-w-0 flex-1">
                  <span class="block font-display text-[14px] font-semibold leading-snug">${esc(short(m))}</span>
          ...
          <div class="mt-6 rounded-[18px] bg-ink p-5 text-white">
            <p class="font-display text-[15.5px] font-semibold tracking-[-0.02em]">Questions about the syllabus?</p>
            <p class="mt-1.5 text-[13.5px] leading-relaxed text-white/60">Talk to an SMEC advisor about the programme, batches and fees.</p>
