...
<header data-header class="site-header fixed inset-x-0 top-0 z-50 border-b border-transparent">
  <div class="header-inner container-x flex items-center justify-between gap-6">
    <a href="#top" aria-label="SMEC Technologies – home" class="inline-flex shrink-0 items-center">${logoImg("dark", 36, 'fetchpriority="high"')}</a>

    <nav aria-label="Primary" class="hidden items-center gap-0.5 lg:flex">
      ${nav
        .map(
          (n) =>
            `<a href="${n.href}" data-nav-link="${n.href.slice(1)}" class="nav-link relative whitespace-nowrap rounded-full px-3 py-2 text-[14px] font-medium transition-colors duration-300">${esc(n.label)}<span class="nav-dot absolute bottom-0 left-1/2 size-1 -translate-x-1/2 rounded-full bg-accent" aria-hidden="true"></span></a>`,
        )
        .join("\n      ")}
    </nav>
    ...
    <div class="flex items-center gap-2">
      ...
      <button type="button" data-menu-open aria-label="Open menu" aria-expanded="false" aria-controls="mobile-menu" class="inline-flex size-11 items-center justify-center rounded-full transition-colors hover:bg-surface-2 lg:hidden">${icon("menu", "size-5")}</button>
    </div>
  </div>
</header>
...
<div id="mobile-menu" data-mobile-menu class="fixed inset-0 z-[60] flex flex-col bg-navy-950 text-white" role="dialog" aria-modal="true" aria-label="Menu" hidden>
  ...
  <div class="container-x relative flex h-[76px] items-center justify-between">
    <a href="#top" aria-label="SMEC Technologies – home" class="inline-flex shrink-0 items-center">${logoImg("light", 34)}</a>
    <button type="button" data-menu-close aria-label="Close menu" class="inline-flex size-11 items-center justify-center rounded-full text-white hover:bg-white/10">${icon("x", "size-5")}</button>
  </div>
