...
<section id="top" data-hero class="hero-wash relative isolate overflow-hidden">
  ...
  <div class="container-x relative grid items-center gap-8 pb-12 pt-20 lg:grid-cols-[minmax(0,1fr)_400px] lg:gap-8 lg:pb-14 lg:pt-24 xl:grid-cols-[minmax(0,1fr)_440px] xl:gap-10">
    <div>
      ...
      <h1 class="mt-7 font-display text-[clamp(2rem,3.3vw,3rem)] font-bold leading-[0.98] tracking-[-0.05em] text-ink">
        ${hero.titleLines
          .map(
            (line) =>
      ...
      <p data-hero-fade class="mt-7 max-w-xl font-display text-[15.5px] font-semibold leading-snug text-ink sm:text-[15.5px]">${esc(course.title)}</p>
      <p data-hero-fade class="mt-3 max-w-xl text-[14.5px] leading-[1.7] text-muted sm:text-[15px]">${esc(hero.valueProp)}</p>
      ...
      <ul data-hero-fade class="mt-8 flex flex-wrap items-center gap-x-7 gap-y-3 text-[13.5px] font-medium text-muted" aria-label="Recognition">
...
<div class="relative z-10 bg-white pt-2">
  <div class="container-x">
    <ul data-hero-fade data-infobar class="-mt-6 grid overflow-hidden rounded-[20px] bg-white text-ink shadow-[0_30px_70px_-40px_rgba(11,16,38,0.4)] ring-1 ring-line sm:grid-cols-2 lg:grid-cols-4" aria-label="Course facts">
      ${infoBar
        .map(
          (f, i) => `<li class="flex items-center gap-4 p-5 sm:p-6 xl:px-9 ${INFO_BORDERS[i] ?? ""}">
        <span class="inline-flex size-10 shrink-0 items-center justify-center rounded-2xl bg-accent-50 text-accent ring-1 ring-accent-100">${INFO_ICONS[f.icon] ?? ""}</span>
        <span class="min-w-0">
          <span class="flex items-center gap-2 font-display text-[15px] font-bold uppercase leading-tight tracking-[-0.01em] text-ink xl:text-[15.5px]">${f.value === "UAE" ? flagUae("h-[12px] w-[18px]") : ""}${esc(f.value)}</span>
...
<div class="relative bg-white pb-3 pt-10 sm:pt-12">
  ...
  <div aria-hidden="true">
    ${marquee(
      marqueeWords
        .map(
          (w, i) =>
            `<span class="flex items-center gap-8 whitespace-nowrap font-display text-[clamp(1.3rem,2.2vw,2rem)] font-bold uppercase leading-none tracking-[-0.035em] ${i % 2 ? "text-outline text-ink/30" : "text-ink"}">${esc(w)}<span class="size-2.5 rotate-45 rounded-[2px] bg-accent" aria-hidden="true"></span></span>`,
        )
        .join(""),
      { gap: "gap-8 pr-10" },
    )}
