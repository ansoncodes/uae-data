...
| # | Section | Id | Notes |
| --- | --- | --- | --- |
| 1 | Hero | `#top` | "Become a Data-Driven Professional", Enquire Now + View Course Details, and the Book a Free Demo Class form |
...
| 4 | Course snapshot | `#course` | Overview (full text behind "Read more"), counts computed from the content, 10 key features |
| 5 | About SMEC | `#about` | Editorial "Building future-ready talent" + count-up statistics |
| 6 | Why SMEC | `#why` | Six large interactive cards in a staggered layout |
| 7 | What you will learn | `#learn` | Collect → Analyse → Visualise → Predict → Automate roadmap (all 9 course outcomes) |
| 8 | Curriculum explorer | `#curriculum` | Desktop: module navigation + panel with progress. Mobile: module accordions |
| 9 | Practical learning | `#projects` | Animated RAW DATA → PYTHON → SQL → POWER BI → AI INSIGHTS pipeline, code/dashboard/prompt cards, compulsory projects |
| 10 | Learning experience | `#experience` | Live sessions, labs & projects, career preparation |
| 11 | Tools & technologies | `#tools` | 18 tools with category filter |
| 12 | Career outcomes | `#career` | "From learning to career" image band, supported career paths, UAE salary chart, SMEC path, 7-step roadmap |
| 13 | Certification | `#certification` | Illustrative certificate with tilt + shine |
| 14 | Trust & recognition | `#proof` | Accreditations; genuine testimonials only (see below) |
| 15 | FAQ | `#faq` | Accordion |
| 16 | Enquiry | `#enquire` | Navy panel: next-step copy and contact links beside the same demo-class form |
| 17 | Final CTA + footer | — | Dubai dusk skyline |
...
assets/js/main.js          behaviour: smooth scroll, header, hero sequence, reveals, counters, parallax,
...
src/sections/*.mjs         one file per page block (head, header, hero, course, learn, practice, career, convert)
...
- **Testimonials are not shown.** The four quotes in `src/content.mjs` are placeholders (`placeholder: true`) and are
  ...
  in `#proof` automatically. `npm run build` prints a note while placeholders remain.
- **Career paths** show only roles from SMEC's job-role list that this curriculum supports: Data Analyst, Business
  Analyst, Data Scientist, Machine Learning Specialist and AI Specialist (`careerPaths`). The full list stays in
  `jobRoles`. The matching FAQ answer uses the same five roles.
- **Statistics** (`about.stats`: 25 years, 2,000+ learning students, 50,000+ placed students, 50+ mentors,
  200+ employees, 50+ courses) come from existing SMEC material — confirm they are current.
...
- **Illustrative visuals** — the pipeline, code, dashboard and prompt cards use sample data and are labelled as such;
  the certificate is marked as an illustrative design.
